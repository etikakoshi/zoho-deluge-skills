...
### クライアントスクリプトと承認中レコード取得時の注意（fetchById / REST API）

一部の環境で、**承認プロセス中（「申請中」フェーズ等）のレコード**について、

- クライアントスクリプト（ZDK）の `fetchById` で取得すると **`undefined` や空データになる場合がある**
- 同じレコードを **REST API（Connections.invoke 経由の CRM API）** で取得すると **問題なく取得できる**

という事象が確認されている（Zoho 公式では GET 制限は明記されていないが、実運用上そう振る舞うケースがある）。

**推奨回避策**

- **1. `fetchById` ではなく REST API（Connections.invoke）で取得する**
  - 例: 契約のレイアウト取得は `GET Contract/{id}?fields=Layout` を REST API で実行する
  - `fetchById` が空を返しても、REST API では取得できるケースがある
- **2. 可能ならサーバー側 Deluge 経由で取得する**
  - クライアントから直接 ZDK で取得せず、ボタン関数やワークフロー関数などサーバー側 Deluge でレコードを取得し、必要な情報だけを戻り値でクライアントに渡す
